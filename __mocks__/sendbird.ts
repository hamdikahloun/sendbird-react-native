...
export function restoreAllDefaultMocks() {
  ...
  mockCreateOpenChannel.mockImplementation(
    (channelName, coverUrl, data, cb) => {
      channel = {
        ...channel,
        name: channelName,
      };
  ...
  mockCreateChannelWithUserIds.mockImplementation(
    (inviteUserIdList, isDistinct, cb) => {
      cb(channel, null);
    },
  );
...
export const mockUpdateCurrentUserInfo = jest.fn((nickname, profileUrl, cb) =>
  cb(user, null),
);
...
const methods = {
  getInstance: jest.fn(() => ({
    ...
    OpenChannel: {
      ...
      createOpenChannelListQuery: jest.fn(),
    },
    GroupChannel: {
...
jest.doMock('sendbird', () => {
  const mock = jest.fn().mockImplementation(() => {
    return methods;
  }) as jest.Mock<any, any> & {getInstance: typeof methods.getInstance};
